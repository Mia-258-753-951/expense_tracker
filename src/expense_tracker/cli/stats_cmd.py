...
def month_stats(month: str = typer.Option(..., '--month')):
    ...
    last_day = calendar.monthrange(parse_month(month).year, parse_month(month).month)[1]
    end_date = parse_month(month).replace(day=last_day)
    ...
    header = (f'{parse_month(month).year}-{parse_month(month).month} stats:')
    typer.echo(header)
...
def stats_range(
    ...
    if by == None:
        ...
        col_w = 12
        header = (
            f'{"CATEGORY":<{label_w}}{"TOTAL(€)":>{col_w}}{"%":>{col_w}}{"COUNT":>{col_w}}'
            )        
        typer.echo(header)
        typer.echo('-' * len(header))
        for k in stat:
        ...
        col_w = 12
        header = (
            f'{"WALLET":<{label_w}}{"TOTAL(€)":>{col_w}}{"%":>{col_w}}{"COUNT":>{col_w}}'
            )        
        typer.echo(header)
        typer.echo('-' * len(header))
        for k in stat:
